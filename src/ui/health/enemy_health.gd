...
func _ready() -> void:
	camera = get_viewport().get_camera_3d().get_node("Control/SubViewportContainer/SubViewport/MainCamera")
	enemy = get_parent()
...
func _process(delta: float) -> void:
	...
	print("var distance: " + str(distance) + " 2) " + str(1.0-distance) + " 3) " +  str(20.0/distance))
