	...
	for z: int in range(params.points_in.y):
		for x: int in range(params.points_in.x):
			data.points[Vector2(x, z)] = Vector2(
				(x) * _get_step_size_x() + params.start.x, (z) * _get_step_size_z() + params.start.y
			)


func _randomize_points() -> void:
	for z: int in range(
		params.randomization_margin, params.points_in.y - params.randomization_margin
	):
		for x: int in range(
			params.randomization_margin, params.points_in.x - params.randomization_margin
		):
